...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Failed to fetch api manifest, error: ", err)
		os.Exit(1)
	}
	// Parse API manifest
	var manifest map[string]string
	if err = json.Unmarshal(res.Body, &manifest); err != nil {
		fmt.Fprintf(os.Stderr, "Failed to parse api manifest, error: ", err)
		os.Exit(1)
	}
	...
	for name, referenceURL := range manifest {
		...
		if err2 != nil {
			fmt.Fprintf(os.Stderr, "Failed to fetch API: ", name, " error: ", err2)
			os.Exit(1)
		}
		// Parse reference
		var s definitions.Service
		if err2 := json.Unmarshal(res.Body, &s); err2 != nil {
			fmt.Fprintf(os.Stderr, "Failed parse API: ", name, " error: ", err2)
			os.Exit(1)
		}
		services[name] = s
	}
	...
	fetchSchema := func(url string) {
		...
		if err2 != nil {
			fmt.Fprintf(os.Stderr, "Failed to fetch: ", url, " error: ", err)
			os.Exit(1)
		}
		// Test that we can parse the JSON schema (otherwise it's invalid)
		var i interface{}
		if json.Unmarshal(res.Body, &i) != nil {
			fmt.Fprintf(os.Stderr, "Failed to parse: ", url, " error: ", err)
			os.Exit(1)
		}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "go fmt, code generation failed, error: ", err)
		os.Exit(1)
	}

	if err := ioutil.WriteFile("services.go", source, 0664); err != nil {
		fmt.Fprintf(os.Stderr, "Failed to save services.go, error: ", err)
		os.Exit(1)
	}
}
